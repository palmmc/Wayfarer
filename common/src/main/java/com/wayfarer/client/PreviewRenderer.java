...
public class PreviewRenderer {
    ...
    public static void render(GuiGraphics graphics, int panelX, int panelY, int panelWidth) {
        ...
        graphics.pose().translate((float) centerX, (float) baseY, 0.0f);
        graphics.pose().scale(finalWPScale, finalWPScale, 1.0f);

        // Draw player head icon for preview (default to steve if clientplayer skin is
        // inaccessible)
        ResourceLocation skinPath = ResourceLocation.fromNamespaceAndPath("minecraft",
                "textures/entity/player/wide/steve.png");
        if (mc.player instanceof net.minecraft.client.player.AbstractClientPlayer clientPlayer) {
            try {
                skinPath = clientPlayer.getSkin().texture();
            } catch (Exception e) {
            }
        }

        graphics.blit(RenderType::guiTextured, skinPath, -7, -20, 8.0f, 8.0f, 14, 14, 8, 8, 64, 64, color);
        ...
        graphics.drawString(mc.font, label, -labelWidth / 2, 0, color, false);
        ...
        ResourceLocation chickenSprite = ResourceLocation.fromNamespaceAndPath("wayfarer", "chicken");
        graphics.blitSprite(RenderType::guiTextured, chickenSprite, chickenX, chickenY, chickenSize, chickenSize,
                color);
    }
}

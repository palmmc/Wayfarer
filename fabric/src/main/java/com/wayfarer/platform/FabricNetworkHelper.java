...
public class FabricNetworkHelper implements INetworkHelper {
    @Override
    public void sendWaypoint(ServerPlayer player, String name, BlockPos pos, Identifier icon, int color, WaypointType type, LocatorType locatorType) {
    ...
    public void clearWaypoints(ServerPlayer player) {
        if (ServerPlayNetworking.canSend(player, S2CWaypointPacket.TYPE)) {
            ServerPlayNetworking.send(player, new S2CWaypointPacket("", BlockPos.ZERO, Identifier.fromNamespaceAndPath("minecraft", "air"), 0, WaypointType.STANDARD, LocatorType.STANDARD, true));
        }
    }
}
